...
export default function HomePage() {
  ...
  const handleProjectHover = (projectId: number | null, hoveredElement: HTMLElement | null) => {
    ...
    if (projectId !== null && hoveredElement) {
      ...
      const isVideo = project.gifUrl.includes('.mp4') || project.gifUrl.includes('.webm');
      ...
      const handleLoad = () => {
        ...
        if (currentMediaIdRef.current === mediaId && currentMediaRef.current === newMedia) {
          gsap.to(newMedia, {
            opacity: 1,
            duration: 0.5,
            ease: "power2.out"
      ...
      if (isVideo) {
        ...
        // Add error handler
        newMedia.addEventListener('error', () => {
      ...
      {projectsData.map((project, index) => {
        ...
        return (
          <Link
            ...
            style={position}
            onMouseEnter={(e) => handleProjectHover(project.id, e.currentTarget)}

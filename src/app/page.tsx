...
export default function HomePage() {
  ...
  const createAmbientMotion = useCallback((immediate = false) => {
  ...
  // Centralized function to handle media and text animations
  const handleMediaAndTextHover = useCallback(
    (
      ...
      if (projectToDisplay) {
        ...
        // Enhance the hovered project title, or fade out logo if a project is hovered
        if (hoveredElement) {
          ...
          // If a project is hovered, fade out the logo
          gsap.to(logoElement, {
            opacity: 0,
            filter: "blur(4px)",
            scale: 0.95,
          ...
          // If logo is hovered, fade out all project titles
          allTitles.forEach((title) => {
            gsap.to(title, {
              opacity: 0,
              filter: "blur(8px)",
          ...
          // Ensure logo is visible and enhanced if it's the target
          gsap.to(logoElement, {
            opacity: 0.9, // Adjust as needed for logo hover state
            filter: "blur(0px)",
            scale: 1.05, // Slight scale for logo hover
            duration: 0.3,
            ease: "power2.out",
          });
        }
        ...
        const isVideo =
          projectToDisplay.videoUrl.includes(".mp4") ||
          projectToDisplay.videoUrl.includes(".webm");
        let newMedia: HTMLVideoElement | HTMLImageElement;

        if (isVideo) {
          newMedia = document.createElement("video");
          newMedia.src = projectToDisplay.videoUrl;
          newMedia.muted = true;
          newMedia.loop = true;
        ...
        const handleLoad = () => {
          ...
          if (
            ...
            if (isVideo) {
              (newMedia as HTMLVideoElement).play();
            }
          } else {
    ...
    [cleanupMedia, createAmbientMotion],
  );

  // Project hover handler
  const handleProjectHover = useCallback(
    (projectId: number | null, hoveredElement: HTMLElement | null) => {
      const project = projectId
        ? limitedProjects.find((p) => p.id === projectId)
        : undefined;
      handleMediaAndTextHover(project, hoveredElement);
      setHoveredProject(projectId);
    },
    [limitedProjects, handleMediaAndTextHover],
  );

  // Logo hover handler
  const handleLogoHover = useCallback(
    (isHovering: boolean) => {
      handleMediaAndTextHover(isHovering ? logoProject : undefined, null);
    },
    [logoProject, handleMediaAndTextHover],
  );
